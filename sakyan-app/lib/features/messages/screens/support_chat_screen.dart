...
// ── Provider: support messages ────────────────────────────────────────────────
final supportMessagesProvider =
    AsyncNotifierProvider<_SupportNotifier, List<MessageModel>>(
        _SupportNotifier.new);

class _SupportNotifier extends AsyncNotifier<List<MessageModel>> {
  @override
  Future<List<MessageModel>> build() async {
    return ref.read(messageRepositoryProvider).getSupportMessages();
  }

  Future<void> send(String content, {String? imageUrl}) async {
    final current = state.value ?? [];
    final msg = await ref
        .read(messageRepositoryProvider)
        .sendSupportMessage(content, imageUrl: imageUrl);
  ...
  Future<void> silentRefresh() async {
    try {
      final msgs =
          await ref.read(messageRepositoryProvider).getSupportMessages();
      final current = state.value;
      if (current == null ||
          current.length != msgs.length ||
          (msgs.isNotEmpty &&
              current.isNotEmpty &&
  ...
  Future<void> refresh() async {
    ...
    state = await AsyncValue.guard(
        () => ref.read(messageRepositoryProvider).getSupportMessages());
  }
}
...
class _SupportChatScreenState extends ConsumerState<SupportChatScreen> {
  ...
  void initState() {
    ...
    _pollTimer = Timer.periodic(const Duration(seconds: 5), (_) {
      if (mounted) {
        ref.read(supportMessagesProvider.notifier).silentRefresh();
      }
    });
  }
  ...
  Future<String?> _uploadImage() async {
    ...
    try {
      ...
      return await SupabaseService.uploadChatImage(
          fileName, _pickedImageBytes!);
    } catch (e) {
  ...
  Future<void> _send() async {
    ...
    try {
      await ref
          .read(supportMessagesProvider.notifier)
          .send(savedText, imageUrl: imageUrl);
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Refresh',
            onPressed: () =>
                ref.read(supportMessagesProvider.notifier).refresh(),
          ),
        ],
      ),
      body: Column(
              ...
              data: (messages) {
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Ask questions, report issues, or get help with your bookings. We\'re here to help!',
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Column(
                      children: [
                        if (showTime) _TimeDivider(time: msg.createdAt, textMuted: textMuted),
                        _SupportBubble(
                          message:    msg,
                          isMe:       isMe,
                          cardColor:  cardColor,
...
class _SupportBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final senderLabel = isMe ? '' : 'Sakyan Support';
