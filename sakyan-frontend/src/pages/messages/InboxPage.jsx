...
import {
  Send, MessageSquare, ArrowLeft, X, ShieldCheck, Paperclip,
  ImageIcon, AlertCircle, RefreshCw, Clock, MessageCircle,
} from 'lucide-react'
import { useNavigate, useSearchParams } from 'react-router-dom'
import {
  useConversations, useMessages, useSendMessage,
  useSupportMessages, useSendSupportMessage,
  useInquiryMessages, useSendInquiry, useSendInquiryReply,
...
function isInquiryConv(conv) {
  return conv?.type === 'inquiry' ||
    conv?.is_inquiry === true ||
  ...
  return (
    <>
      <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} ${isOptimistic ? 'opacity-70' : ''}`}>
        <div className={`max-w-[75%] rounded-2xl text-sm leading-relaxed shadow-sm overflow-hidden
          ${isOwn
...
export default function InboxPage() {
  ...
  const filteredConvs = convList.filter(c => {
    ...
    if (activeFilter === 'inquiry') return isInquiryConv(c)
    if (activeFilter === 'booking') return !isSupportConv(c) && !isInquiryConv(c)
    return true
  })

  const filterCounts = {
    all:     convList.filter(c => c.unread_count > 0).length,
    booking: convList.filter(c => !isSupportConv(c) && !isInquiryConv(c) && c.unread_count > 0).length,
    inquiry: convList.filter(c => isInquiryConv(c) && c.unread_count > 0).length,
  ...
  const { data: bookingMessages, isLoading: bookingMsgsLoading } =
    useMessages(!isSupport && !isInquiry && activeConv ? activeConv.booking_id : null)

  const { data: supportMessages, isLoading: supportMsgsLoading } =
    useSupportMessages(isSupport ? adminSupportPartnerId : undefined)

  const { data: inquiryMessages, isLoading: inquiryMsgsLoading } =
    useInquiryMessages(isInquiry ? activeInquiryPartnerId : null)
  ...
  const handleSend = useCallback(async (e, retryData = null) => {
    ...
    if (isInquiry) {
      ...
      if (receiverId) {
        ...
        sendInquiryMsg.mutate({ partnerId, content, imageUrl }, { onError })
      }
    } else if (isSupport) {
      sendSupportMsg.mutate(
        { content, imageUrl, ...(adminSupportPartnerId ? { receiverId: adminSupportPartnerId } : {}) },
        { onError }
      )
    } else {
      ...
      activeInquiryPartnerId, user, sendSupportMsg, sendBookingMsg,
      sendInquiryMsg, sendInquiryReply, uploadFile])
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f1117]">
      ...
      <div className="sticky top-0 z-20 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-4 py-3.5 flex items-center justify-between">
        ...
        {activeConv && mobileView === 'chat' && (
          <button onClick={() => setMobileView('list')} className="sm:hidden p-2 rounded-xl text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
            <X size={18} />
          </button>
        )}
      </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-16 px-6 text-center">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 leading-relaxed">
                {activeFilter === 'inquiry'
                  ? 'Click "Message Partner" on any car listing to start an inquiry.'
            ...
            <>
              ...
              <div className={`px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center gap-3
                ...
                {isSupport
                  ...
                  : isInquiry
                    ? <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shrink-0"><MessageCircle size={15} className="text-white" /></div>
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isInquiry && (
                      <span className="text-[10px] bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-400 px-1.5 py-0.5 rounded-full font-medium">Inquiry</span>
                    )}
                  </div>
